...
function generateLevel() {
    ...
    for (i=0; i <= platformPieces; i++) {
        // Get structure type from predefined map (-1 = hole, 0-5 = structure type)
        const mapValue = i < mapData.length ? mapData[i] : 0;
        const shouldCreateHole = mapValue === -1;
        
        // Check if we should skip (safe zones at start and end, or forced hole)
        const isInSafeZoneStart = pieceStart <= screenWidth * 2;
        const isInSafeZoneEnd = pieceStart >= worldWidth - screenWidth * 2;
        const isNearCastle = pieceStart >= (worldWidth - screenWidth * (isLevelOverworld ? 1 : 1.5));
        const skipHole = isInSafeZoneStart || isInSafeZoneEnd || isNearCastle || lastWasHole > 0;

        if (!shouldCreateHole || skipHole) {
            lastWasHole--;
            ...
            if (!isNearCastle && pieceStart > (screenWidth + platformPiecesWidth * 2) && lastWasHole < 1 && lastWasStructure < 1 && !isInSafeZoneStart) {
                // Use predefined structure or pass it to generateStructure
                if (mapValue >= 0 && mapValue <= 5) {
                    lastWasStructure = generateStructureFromMap.call(this, pieceStart, mapValue);
                } else {
                    lastWasStructure = 1;
                }
            }
        ...
        } else if (shouldCreateHole && !skipHole) {
